...
//	}
	...
	@Override
	public List<Object[]> getMSISDNsByRegle2(Long regleId, int page, int pageSize) {
	    try {
	        String subQueryString = "(SELECT max(valeur) FROM stat.regle_parametres_valeur_seq as b "
	                + "WHERE a.msisdn = b.msisdn and id_regle = " + regleId + " GROUP BY id_parametre, id_regle)";
	        ...
	        String queryString;
	        if (maxvalue != null && maxvalue.size() <= 1) {
	            queryString = "SELECT id_regle, msisdn,"
	                    + " to_timestamp(min(substr(date_debut,1,8)), 'YYMMDDHH24MISS') as date_debut,"
	                    + " to_timestamp(max(substr(date_fin,1,8)), 'YYMMDDHH24MISS') as date_fin,"
	                    + " max(date_detection) as date_detection, count(*) as nb_occurance,"
	                    + subQueryString + " as maxValeur FROM stat.alerte_fraude_seq a"
	                    + " WHERE id_regle = :regleId GROUP BY id_regle, msisdn ORDER BY date_detection DESC";
	        } else {
	            queryString = "SELECT id_regle, msisdn,"
	                    + " to_timestamp(min(substr(date_debut,1,8)), 'YYMMDDHH24MISS') as date_debut,"
	                    + " to_timestamp(max(substr(date_fin,1,8)), 'YYMMDDHH24MISS') as date_fin,"
	                    + " max(date_detection) as date_detection, count(*) as nb_occurance"
	                    + " FROM stat.alerte_fraude_seq a WHERE id_regle = :regleId GROUP BY id_regle, msisdn"
	                    + " ORDER BY date_detection DESC";
	        }

	        int startPosition = (page - 1) * pageSize;
	        return emRep.createNativeQuery(queryString)
	                .setParameter("regleId", regleId)
	...
	public List<Object[]> getAllRulesWithMaxDateDetection() {
	    String query = "SELECT " +
	            "r.ID, " +
	            "r.nom, " +
	            "ad.max_date_detection " +
	            "FROM " +
	            "tableref.regles_fraudes r " +
	            "LEFT JOIN ( " +
	            "SELECT " +
	            "id_regle, " +
	            "MAX(date_detection) AS max_date_detection " +
	            "FROM " +
	            "stat.alerte_fraude_seq " +
	            "GROUP BY " +
	            "id_regle " +
	            ") ad ON r.ID = ad.id_regle " +
	            "WHERE " +
	            "r.ID <> 0 " +
	            "ORDER BY " +
	            "r.nom";
...
//			existingFilter.setFiltreFraude(updateRequest.getFiltreFraude());
	...
	@Override
	public List<Object[]> getMsisdnAndRegleCount2(int page, int pageSize) {
	    String jpql = "SELECT msisdn, COUNT(*) FROM (SELECT msisdn, id_regle, COUNT(*) AS nombre FROM stat.alerte_fraude_seq GROUP BY msisdn, id_regle) a GROUP BY msisdn HAVING COUNT(*) > 1";

	    Query query = emRep.createNativeQuery(jpql);
